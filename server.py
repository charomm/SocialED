...
def home():
    ...
    if 'user_name' not in session:
        return process_error("You must be logged to use the app / Debe registrarse antes de usar la aplicación",
                             url_for("login"))
...
def load_messages_from_user(user):
    ...
    with open(file_path, 'r') as f:
        data = json.load(f)
    messages_with_author = [(data["user_name"], message[0], message) for message in data["messages"]]
